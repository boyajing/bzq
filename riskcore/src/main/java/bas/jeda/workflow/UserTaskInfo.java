...
public abstract class UserTaskInfo implements IUserTaskInfo {

    protected Logger logger = Logger.getLogger(UserTaskInfo.class);
    ...
    protected UserTaskInfo() {

    }

    protected UserTaskInfo(org.activiti.engine.task.TaskInfo task, IFlowHandler handler) {
...
//        this.localVariables = new HashMap<String, Object>();
        ...
        //----------------------------------------------
        // 这里占时保留，流程启动时在流程变量中设置一个变量 startDate 等这一批流程结束之后改变去掉查询数据库逻辑  增加 this.startDate = this.variables.get("startDate");
        this.startDate = (Date)this.variables.get("startDate");
        if(startDate == null){
...
//        this.processName = this.handler.getProcessDefinition(processDefinitionId).getName();
...
//        fillVariables();
        ...
        //----------------------------------------------
        // 这里占时保留，流程启动时在流程变量中设置一个变量 businessKey 等这一批流程结束之后改变去掉查询数据库逻辑  增加 this.businessKey  = this.variables.get("businesskey");
        this.businessKey  =(String) this.variables.get("businesskey");
