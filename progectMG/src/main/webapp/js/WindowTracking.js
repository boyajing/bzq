...
function erase(pos) {
    if (pos >= 0) this.splice(pos, 1); else this.splice(0, this.length);
    return this;
}
...
function urlIncQuerystring(str) {
    tmp = str.toString();
    tmp = tmp.toLowerCase();
    ePos = str.indexOf(".asp");
    sPos = str.lastIndexOf("/", ePos);
...
function nullOpeners() {
    ...
    if (aTPWin != null) {
        for (i = 0; i < aTPWin.length; i++) {
            if ((aTPWin[i]) && (!aTPWin[i].closed)) {
                if (aTPWin[i].owner == document.URL) {
                    aTPWin[i].opener = null;
                    aTPWin[i].owner = null;
                }
            }
        }
...
//指标长度控制
function byteLength(sStr, sLen) {
    var flag = false;
    if (sStr.length) {
        for (var i = 0; i < sStr.length; i++) {
            var dValue;
            dValue = sStr[i].value.toString().replace(/,/g, "");
            var aMatch = dValue.match(/[^\x00-\x80]/g);
            if (dValue.length + (!aMatch ? 0 : aMatch.length) > sLen) {
                flag = true;
                break;
            }
        }
        ...
        var dValue;
        dValue = sStr.value.toString().replace(/,/g, "");
        var aMatch = dValue.match(/[^\x00-\x80]/g);
        if (dValue.length + (!aMatch ? 0 : aMatch.length) > sLen) {
            flag = true;
        }
