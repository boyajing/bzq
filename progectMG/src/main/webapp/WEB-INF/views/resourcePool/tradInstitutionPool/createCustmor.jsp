...
<head>
    ...
    <script type="text/javascript">
        ...
        //保存
        function save() {
            var count=$("#creditTable tr").length;
            if(count==2){
                alert("没有授信信息，不能保存");
            }else {
                var num = 0;
                for (var i = 2; i < count; i++) {
                    var  perAmt= $("#creditTable tr").eq(i).children('td').eq(0).find("select").val();
                    var actAmt= $("#creditTable tr").eq(i).children('td').eq(1).find("input").val();
                    var bgDate= $("#creditTable tr").eq(i).children('td').eq(2).find("input").val();
                    var endDate = $("#creditTable tr").eq(i).children('td').eq(3).find("input").val();
                    if (perAmt != "" && actAmt != "" && bgDate != "" && endDate != "") {
                        num++;
                    } else if (putdate == "" ) {
                        alert("授信类别不能为空！");
                        break;
                ...
                if (count-2 == num) {
                    $.ajax({
                        url:'<%=path%>/tradInstitutionPool/save',
                ...
                <table id="creditTable">
                    ...
                    <tr>
                        ...
                        <td colspan="4" style="text-align:right;">
                            ...
                            <%--<button type="button" id="delete" onclick="deleteCredit()">删除</button>--%>
                        </td>
                    </tr>
                    <tr>
                        <td>授信类别</td>
