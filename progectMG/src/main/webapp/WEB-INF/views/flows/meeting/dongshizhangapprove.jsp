...
<script type="text/javascript">
    ...
    function onloadc(issueId) {
        ...
        <%--if(isdone2==true){--%>
            <%--openWin({--%>
                ...
                <%--&lt;%&ndash;url:"<%=path%>/fileManager/index?issueId="+issueId+"&type=4&meetingId=${meetingId}"&ndash;%&gt;--%>
                <%--, width: screen.width--%>
                <%--, height: screen.height-100--%>
            <%--});--%>
        <%--}else {--%>
    ...
    function reback() {
        ...
        $.ajax({
            type: 'POST',
            url: '<%=path%>/flow/takeBack/'+flowid+'/'+taskId+'/dongshizhang/huiyimishuappprove.json',
            dataType: 'json',
            async: false,
            data :{},
            success: function (data) {
        ...
        <table id="votetable">
            ...
            <c:forEach var="item" items="${biaojuelist}" varStatus="status">
                <tr>
                    ...
                    <td width="20%" style="text-align: left">应参会委员${item.peopleNum}人，实参会委员${item.peopleNum-item.weibiaojuenum}人。
                        同意${item.tongyinum}人，再议${item.xvyinum}人，否决${item.foujuenum}人。
                        表决结果为：${item.result}。
                    </td>
                    <td width="50%">
                        <textarea  type="text" name="boardOpinion_${item.issueId}"  cols ="300" rows="10" maxlength="999" onscroll= "this.rows=this.rows+1"
                                 style="word-wrap:break-word; width:90%; resize: none; font-size: 16px;">${item.boardOpinion}</textarea>
                    </td>
                    <td width="10%">
            ...
            <div class="container-fluid">
                <c:forEach var="item" items="${biaojuelist}" varStatus="status">
                    ...
                    <tr>
                        ...
                        <td>
                            <textarea  type="text" cols ="300" rows="10" onscroll= "this.rows=this.rows+1" readonly
                                    style="word-wrap:break-word;width: 90%;resize: none;font-size: 16px;">${item.zongjingliOpinion}</textarea>
                            </textarea>
                        </td>
                    </tr>
                </c:forEach>
