...
<script type="text/javascript">
    ...
    $(function(){
        ...
        if(window.opener){
            var id=$('#meeting_issue_tbody');
            window.opener.yiti(id);
        }
                    ...
                    success:function(data){
                        if(data.sucess == "sucess"){
                            ...
                            openWin({
                                ...
                                url:"<%=path%>/flow/approve/"+data.taskId
                                , width: screen.width
...
<div class="container-fluid">
        ...
        <form id="meetingForm" action="<%=path%>/meeting/start" method="post">
            ...
            <table class="well table resultTable table-hover">
                <tr>
                    ...
                    <td>会议地点</td>
                    <td><input type="text" name="meetingAdress" value="${meetingBase.meetingAdress}"/></td>
                </tr>
                <tr>
                ...
                <tr>
                    <%--<td>会议主持人</td>
                    <td>
                        <input type="text" id="meetingEmcee_CN" value="${meetingEmcee}" readonly/>
                        <input type="hidden" id="meetingEmcee_EN" name="meetingEmcee" value="${meeting.meetingEmcee}"/>
                        <button type="button" onclick="selectuser('<%=UserRoleDefine.ROLE_JCW_ZZ%>')">选人</button>
                    </td>--%>
                    <td>会议委员</td>
                    <td colspan="3">
            ...
            <table id="itemTable">
                ...
                <%--<tr>--%>
                <%--<td>${vs.count}</td>--%>
                <%--<td><input type="hidden" name="schemeId" value="${item.issueBusinssKey}" />--%>
                <%--<a href='javascript:void(0)' onclick='openFaxx("${item.issueBusinssKey}")'>${item.issueName}</a></td>--%>
                <%--<td><button type="button" onclick="deleteRow(this,'${item.issueName}','${item.issueBusinssKey}')">删除</button></td>--%>
                <%--&lt;%&ndash;this,,${item.issueBusinssKey}&ndash;%&gt;--%>
                <%--</tr>--%>
                <%--</c:forEach>--%>
                </tbody>
            </table>
    ...
    <div class="modal diagramModal fade hide" id="meeting_issue_modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        ...
        <%--<div class="modal-footer">--%>
        <%--<button id='meeting_issue_btn' class="btn btn-large btn-primary"  data-dismiss="modal" aria-hidden="true">确认</button>--%>
        <%--<button class="btn btn-large" data-dismiss="modal" aria-hidden="true">关闭</button>--%>
        <%--</div>--%>
    </div>
