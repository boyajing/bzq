...
<div class="container-fluid"  style="height:100%;width:96%; position:absolute;top:0;overflow-y: scroll;padding-right:20px ">
    ...
    <div>
        ...
        <table id="dtable">
            <tr>
                <td colspan="8"><h4>合同进度详细信息</h4></td>
            </tr>
            <c:forEach items="${details}" var="detail">
                <tr>
                    <td colspan="7"><h5><nt:workpiecename workpieceno="${detail.workpieceNo}"></nt:workpiecename></h5></td>
                </tr>
                <tr>
                    <td>发生时间</td>
                    <%--<td>合同开始</td>--%>
                ...
                <c:forEach items="${detail.tflows}" var="flow">
                <tr>
                    <td>
                        <fmt:formatDate value="${flow.actDate}" pattern="yyyy-MM-dd" ></fmt:formatDate>
                    </td>
                    <%--<td>--%>
                        <%--<c:if test="${empty flow.beginDate}" >--</c:if>--%>
                        <%--<c:if test="${not empty flow.beginDate}"><fmt:formatDate value="${flow.beginDate}" pattern="yyyy-MM-dd" ></fmt:formatDate></c:if>--%>
                    <%--</td>--%>
                    <td>
                        <c:if test="${empty flow.actQuantity}" >--</c:if>
                        <c:if test="${not empty flow.actQuantity}" >${flow.actQuantity}</c:if>
                    </td>
                    <td>
                        <c:if test="${empty flow.actPickup}" >--</c:if>
                        <c:if test="${not empty flow.actPickup}" >${flow.actPickup}</c:if>
                    </td>
                    <%--<td>--%>
                        <%--<c:if test="${empty flow.actAmt}" >--</c:if>--%>
                        <%--<c:if test="${not empty flow.actAmt}"><fmt:formatNumber pattern="##,#00.00" value="${flow.actAmt}"></fmt:formatNumber></c:if>--%>
                    <%--</td>--%>
                    <%--<td>--%>
                        <%--<c:if test="${empty flow.endDate}" >--</c:if>--%>
                        <%--<c:if test="${not empty flow.endDate}"><fmt:formatDate value="${flow.endDate}" pattern="yyyy-MM-dd" ></fmt:formatDate></c:if>--%>
                    <%--</td>--%>
                    <td>
                        <c:if test="${empty flow.type}" >--</c:if>
                        <c:if test="${not empty flow.type}" ><nt:codeValue index="${flow.type}" ctype="010"></nt:codeValue></c:if>
                    </td>
                </tr>
                </c:forEach>
                <tr>
