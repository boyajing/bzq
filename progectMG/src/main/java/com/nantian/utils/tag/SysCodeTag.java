 ...
 */
public class SysCodeTag extends BodyTagSupport {
           ...
           // Map<String,Object> syscode = (Map<String,Object>)session.getAttribute("sys_code_list");
            ...
            for (TSysCode tSysCode: tSysCodes) {
                        if(tSysCode.getCodeValue().equals(index) ){
                            sb.append("<OPTION value='" + tSysCode.getCodeValue() + "' selected>" + tSysCode.getCodeName() + "</OPTION>");
                        }else{
                            sb.append("<OPTION value='" + tSysCode.getCodeValue() + "' >" + tSysCode.getCodeName() + "</OPTION>");
                        }
                    }
        } catch (Exception e) {
            e.printStackTrace();
