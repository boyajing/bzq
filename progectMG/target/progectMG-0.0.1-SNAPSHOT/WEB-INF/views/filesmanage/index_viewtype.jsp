    ...
    <div>
        <table id="resultTab" class="table table-bordered table-condensed table-striped table-hover">
            ...
            <c:forEach var="item" items="${list}">
                <% i++;%>
                <tr>
                    <td class="box"><input type="checkbox" value="${item.id}" name="radio"/></td>
                    <td><%=i%></td>
                   <%-- <td><a href="<%=path%>/fileManager/fileload?id=${item.id}">${item.fileName}</a></td>--%>
                   <%-- <td><a href="<%=path%>/contractfliepdf/mfileload?id=${item.id}">${item.fileName}</a></td>--%>

                    <td><a onclick="managerfileload( '${item.id},${item.fileName}')">${item.fileName}</a></td>
...
<script type="text/javascript">
    ...
    function managerfileload(key) {
       ...
       var fileName = arry[1];
            ...
            if(type=="docx"){
                location.href='<%=path%>/contractfliepdf/mfileload?id='+temp+'';//docx文件下载后转为PDE格式
            } else{
                location.href='<%=path%>/fileManager/fileload?id='+temp+'';//除了docx的任何文件下载之后还是原文件
            }
    }
