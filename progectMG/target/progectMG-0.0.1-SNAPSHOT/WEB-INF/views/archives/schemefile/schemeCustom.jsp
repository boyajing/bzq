...
<script type="text/javascript">
    ...
    //查看客户详细信息
    function queryAll(temp,temp1) {
        window.open("<%=path%>/fileManager/index_view?businessType=file_3&businessId="+temp, "担保人核保", "height=800,width=1500,top=100,left=300,toolbar=no,menubar=no,scrollbars=yes, resizable=1,location=no, status=no");
        <%--location.href = "<%=path%>/fileManager/index_view?businessType=file_3&businessId="+temp;--%>
    }
    //返回
    function goback() {
        window.location.href="<%=path%>/archives/schemefile/schemeProject?maxId=${maxId}";
    }
