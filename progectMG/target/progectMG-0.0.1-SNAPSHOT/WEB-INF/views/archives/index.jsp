...
<div class="container-fluid">
    ...
    <div class="row-fluid">
        ...
        <div style="display: none"><input id="imports" type="file" onchange="imports();"/></div>
    </div>
...
<script type="text/javascript">
    ...
    //查询
    function query() {
        $("#queryForm").attr("action", "<%=path%>/project/indexFinish").submit();
    }
    ...
    function queryAll(temp) {
        var url = "<%=path%>/archives/projectfile/projectCustom?maxId="+temp;
        location.href = "<%=path%>/commonfrm?action=add&id=_ARCHIVES01&maxId="+temp+"&url="+url;
        <%--//location.href='<%=path%>/projectInvest/queryAll?projectcode='+temp+'';--%>
        <%--location.href='<%=path%>/project/queryAll?projectcode='+temp+'';--%>
    }
